...
let data_cleaned_3: any; // array of a table column to display in the console when a user hits a dot.
...
let match_owner: string = "click the dots! Amplitude modulation!"; // store the current owner of the match

let sine: any; // our sine wave oscillator
let env: any; // our envelope so we can play the sine as a note
let LFO: any; // our LFO that we use to control the amplitude of the sine
let offsetFrequency: number = 300;

export const sonifyplotampmod: P5jsSketch = (p5, parentRef) => {
  p5.setup = () => {
  ...
  const loadData = () => {
    ...
    data_cleaned_3 = data_cleaned_3.map((str: string) => Number(str));
  };
