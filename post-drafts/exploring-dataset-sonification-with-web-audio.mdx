...
The code block below demonstrates how we can initalize and control a simple sinewave oscillator in p5. The frequency of the sinewave is gradually incremented (from 0 to 10kHz), on every new window frame, resulting in a continuous sine sweep.
...
Knowing this, together with some knowledge of sound synthesis, we can start to build more complex sounds that can tell us more in-depth information about our data.

Below is a sonically upgraded example of our previous red-dot (now blue) coffee plot. The plot features a second oscillator used as an LFO to control the amplitude of the first oscillator, creating an amplitude modulation synthesizer in the process. By listening to the frequency of the LFO, we can disseminate information about how many bags of coffee were produced by the given farm that given year.
