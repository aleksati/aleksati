...
2. <MyLink href="#---">Synchronization With The Kuramoto Model</MyLink>
...
```C
// inside [mc.gen~ ksync_node] codeblock
// Scaling the k_value and n_freq to the system sampling rate using global gen variables
...
Since my Kuramoto model only produces sine waves, I needed a way to convert this into audible metronomes. For some time, I have been interested in machine-learning libraries for Max designed for realtime audio processing. In particular, I have been interested in libraries like SP-tools that give users the ability to build and explore corpora of sound, e.g. large banks/folders of audio samples, and much more.

The first iteration of aleks.ksync comes with a simple system that plays an audio sample at every sine-wave zero-crossing. This playback mechanism is robust and imitates the metronome characteristic perfectly. However, considering I would need 100 metronomes for my projects, I decided to build several audio corpora and explore how I could incorporate clustering techniques to design an interesting audio playback system for my project.

PICTURE..

the sounds to be similar. not many different sounds.
interact and toggle via a 2D matrix

This is what I want to do...

This is how I did it
build a dataset of drum machine claps and claves
first clustering..
then
then SP-tools to build a corpora (with audio example)
Clustering... show the picture.
(used gridmatch etc.)
