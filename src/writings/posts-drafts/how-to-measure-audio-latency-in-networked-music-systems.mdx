...
<MyImage src="audiolatency-irl-nmp-1.png" alt="From Telematic Testing, an event that combined live music and dance using advanced networked technology in three different places, hosted at Popsenteret in Oslo, April 2024. Photo by Thusha Rajendran." caption="From Telematic Testing, an event that combined live music and dance using advanced networked technology in three different places, hosted at Popsenteret, Oslo, April 2024. Photo by Thusha Rajendran." />
...
I prefer to build a measurement systems with a laptop connected to an external audio interface that has two or more inputs/outputs. To create the two identical signal chains, I physically connecting two of the outputs on the interface to two of the inputs with XLR or jack, effectively looping back two channels. Audio is administered then from one end of the chains and recorded at the other.

<MySlideshow imgs={["audiolatency-diagram-loopback.png", "audiolatency-irl-loopback.png"]} captions={["Diagram of the loopback measurement system using a laptop connected to an external audio interface. Looping back channels isolates the signals from any unknown/unwanted processes that affect the latency. Real example in the next image -->", "A loopback measurement system where I loop back two channels on my MOTU M4 audio interface using two 1/4'' jack cables. Audio is administered and the running latency is measured through custom MaxMSP software (more on this later). As you can see, 0 sample latency means our signal chains are subject to equal conditions."]} />
...
<MySlideshow imgs={["audiolatency-diagram-handyrec.png", "audiolatency-irl-handyrec.png"]} captions={["Diagram of a simpler handy recorder measurement system where audio is administered from a laptop or smartphone and recorded by an additional device. Real example in the next image -->", "A handy recorder measurement system with my laptop connected to both inputs on a Zoom H4N recorder using a stereo mini-jack to dual mono 1/4'' jack cable."]} />
...
With the measurement system in place, what remains is simply to introduce the system you wish to measure into one of the signal chains. The latency between the recorded refence and delayed signals is an accurate representation of the latency in the inserted system.

<MySlideshow
  imgs={["audiolatency-diagram-loopback-delay.png", "audiolatency-irl-loopback-delay.png", "audiolatency-irl-handyrec-delay.png"]}
  captions={["The loopback measurement system with added measurement object. Real examples in the next images -->", "Using a Hall Research AD-340 Universal Audio Delay Processor as my measurement object in a loopback measurement system. Notice that the analog delay is set to 194 ms delay, the same latency as my MaxMSP software is registering on screen.", "Likewise, using a Hall Research AD-340 Universal Audio Delay Processor as my measurement object in a handy recorder measurement system. Here, the delay is set to 200ms on the device. Later, my rough analysis in Audacity showed that the sample latency was about 8855 samples. With a 44.1kHz sampling rate, this amounts to 200.7ms (1000/44100*8855=200.7)."]}
/>
...
<MyImage src="audiolatency-diagram-soose.png" alt="OOSE delay diagram showing the typical components included in an NMP signal path. Any additional step along the chain will cause additional latency to various degrees." caption="OOSE delay diagram showing the typical components included in an NMP signal path. Any additional step along the chain will cause additional latency to various degrees." />
...
<MyImage src="audiolatency-diagram-soose-rtt.png" alt="Of course, to be able to measure the true OOSE delay, we need to adopt an RTT measurement strategy and consider the hardware and system configuration settings at each end." caption="Of course, to be able to measure the true OOSE delay, we need to adopt an RTT measurement strategy and consider the hardware and system configuration settings at each end.." />
...
<MyImage src="audiolatency-diagram-soose-rtt-loopback-delay.png" alt="Diagram showing how you can connect the networked music system to the measurement system. Notice that, in this design, you might need additional microphones and speakers at each end to be closest to the OOSE standard. However, it is also to simplify by administering and routing the measurement impulses directly to the mixer. " caption="Diagram showing how you can connect the networked music system to the measurement system. Notice that, in this design, you might need additional microphones and speakers at each end to be closest to the OOSE standard. However, it is also to simplify by administering and routing the measurement impulses directly to the mixer. " />

However, since we can pretty accurately calculate how it takes sound to travel a specific distance through the air, it is also possible to skip the extra mic and speaker stages, administer the measurement impulses directly to the mixer at one end, and route them back from the mixer at the remote end. In this case, just remember to take note of the distances between the speakers and the performers and factor these distances into the final calculations.
