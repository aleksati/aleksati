...
In a recent episode of [Nyskjerrige Norge](https://www.forskningsradet.no/om-oss/forskningskommunikasjon/podkast/), the new podcast by the Norwegian Research Council, host Kristoffer Schau meets professor Mats Carlsson at the Rosseland Centre for Solar Physics, UiO, to discuss the mysteries of our sun. Aside from the interesting contents of the show, a particular comment made by Schau about a TV feed in Carlsson's UiO lunchroom caught my interest. The TV feed was made up of real-time scientific images of the sun and Schau remarked how great it would be if such a feed was available as a screensaver for TVs and laptops. Due to my lifelong interest in space, and more recent venture as an amateur astrophotographer, I was instantly inspired and decided to build the screensaver for myself.

In this post, I present *The Sun Right Now*, my new desktop screensaver that displays real-time 4K images of the sun in a mesmerizing slideshow. What's more, the images are updated automatically every 5 minutes to ensure only the very latest captures are shown. Below, I share how the app works and how you can use it for yourself. 

For a quick preview, check out a live browser version of the screensaver here - [https://aleksati.net/thesunrightnow](https://aleksati.net/thesunrightnow)
...
The Nodejs API has one job; to retrieve image filenames from a public NASA repository that holds images from their Solar Dynamics Observatory (SDO). The SDO is a satellite currently in orbit around Earth whose job is to take photos and continuous measurements of the sun. Read more about the SDO project [on their website](https://sdo.gsfc.nasa.gov/). Fortunately, all the scientific data from NASA SDO is publically available and hosted online. For my screensaver app, this was great news. Since the images are already hosted online, I didn't need to fetch them from some API, I could just retrieve the URLs to the images and load them inside an HTML iframe element.

To get the very latest images on demand, I took advantage of how NASA organizes its SDO images. The photos are put into a folder structure that represents days, months and years. The image filenames themselves also include the date, time, type, and resolution of the images. You can browse the SDO assets folder I use [here](https://sdo.gsfc.nasa.gov/assets/img/browse). With this in place, I only had to add the current date to the fetching URL to find the correct image folder. Then, to get the latest images, I simply fetched all the filenames in the folder and sorted them on my API based on the values in their filenames. Finally, I filtered for only 4k images. 
...
On Win.. Win To make it a screensaver, you just edit the name and .. boom.
