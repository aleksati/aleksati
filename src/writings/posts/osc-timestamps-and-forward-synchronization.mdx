...
<MyLink href="https://opensoundcontrol.stanford.edu/index.html">
  Open Sound Control
</MyLink> (OSC) is a flexible and high-resolution media encoding designed for sending
audio control messages between musical devices in real time, similar to MIDI, originally
developed by researchers Matt Wright and Adrian Freed at CNMAT, UC Berkeley, in 1997
(Schmeder, Freed, 2008). Because of its high-resolution data transfer (64-bit compared
to 8-bit MIDI) and built-in Timestamp support, OSC provides an ideal framework for
exploring creative and new approaches to synchronization and latency management over
networks.
...
{/\* 1. What are OSC Timestamps and Forward Synchronization? 2. Code Setup and Dependencies 3. Step 1 - Simple P2P Messaging 4. Step 2 - Network Controlled Metronome 5. Step 3 - OSC Timestamp Controlled Metronome 6. Step 4 - Forward Synchronized Metronome 7. Sources
