...
**DrDrive target** - The target audio is taken straight out of the dataset. This is just the audio being recorded through the actual DrDrive plugin, usually referred to as the ground truth:
<MyAudioPlayer src="drdrive-uncond-1-tar-64.mp3" />

**DrDrive student network (non-distilled)** - In this scenario, the audio is recorded through a small neural network (student) that models the DrDrive plugin. The network is trained on the dataset where the target audio came from: 
<MyAudioPlayer src="drdrive-uncond-1-pred-nondistilled-64.mp3" />

**DrDrive student network (distilled)** - In this scenario, the audio is also recorded through a small neural network (student) that models the DrDrive plugin, but this time the model has been "taught", or trained, on data produced by a larger teacher model instead of the target dataset:
