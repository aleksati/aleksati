...
**DrDrive student network (non-distilled)** - In this scenario, the audio was recorded through a small student network that models the DrDrive plugin. The network is trained on the dataset where the target audio came from:
<MyAudioPlayer src="drdrive-uncond-1-pred-nondistilled-64.mp3" />

**DrDrive student network (distilled)** - In this scenario, the audio was recorded through another small student network that models the DrDrive plugin, but this network has been "taught", or trained, on data produced by a larger teacher model instead of the target dataset:
