...
The technique most widely used to measure audio latency in a networked music setup is to send out audio impulses through the networked signal chain and compare this delayed signal against some reference impulse (Fasciani, 2020). This is not unique and is a pretty common way to measure latency in any technical system. I would argue that the most critical step in this technique is to make sure to properly isolate the thing you want to measure from other components of the system. In other words, it's important to give your delayed and reference signals the same initial conditions with the same processing from start to finish.

In our context, we can create this signal isolation quite easily by setting up two identical signal chains where you can administer audio from one end and record it at the other. Finally, what remains is simply to introduce the system you wish to measure into the signal chain of the delayed impulse.

[DIAGRAM] og diagram av handy recorder
diagram av RTT OOSE med laptop of med handy recorder.

Let's look at two examples of how to create this kind of measurement system in a real-world NMP scenario. The preferred way is to use a laptop with an external audio interface that has two or more inputs and outputs. To create the two identical signal chains, physically connect two of the outputs on the interface to two of the inputs with XLR or jack, effectively looping back two channels. On the other hand, a cheaper and more travel-friendly system can be designed using something like a smartphone connected to a handy recorder with two or more inputs. To create identical signal chains with this system, split the physical audio output from your smartphone (USB-C to jack) and connect them to two physical inputs on the handy recorder. As you can tell, these two examples are similar but with some key differences.

To measure the latency, send one impulse audio signal to both the reference and delayed signal chain outputs and record them. After, count the sample difference between recorded reference and delayed impulses and factor in the sampling rate to get the latency metric. It's important to be as scientific as possible here and spend a good amount of time verifying that your method can accurately isolate the measurement object consistently before you put it into practice. The latency between the two recorded impulses should be zero if no additional system or module is inserted into the delayed signal chain.

To be able to best distinguish between the reference and delayed sound, it's wise to choose audio impulses that are "short". This means audio that is short in length but also audio that features an immediate attack and sustain, like clapping sounds or, even better, a simple signal click. I also recommend using free and open-source DAWs like Audacity or Reaper to administer and record the audio. It's also possible to use more fancy software like MaxMSP which can automate things and make the measurement process very comfortable.

[BILDE]
measurement system with laptop and AD/DA with looped back (referennce og vis 0 latency i MAX)
measurement system with laptop and AD/DA with looped back (with placeholder and MaxMSP showing the latency)
measurement system with a handy recorder and phone (placeholder for )
...
However, the OOSE delay only carries any real value if measured in a system that can transmit clear and stable audio. In other words, it's practically meaningless to measure the latency of a NMP system that renders terrible audio quality. However, the problem is that enhancing the media fidelity in streaming contexts usually means adding additional processing stages that, in turn, will add more latency. For instance, increasing audio buffersizes and bit-rates, or experimenting with different compression settings, are all good modifications that can enhance audio quality of streams, but they all do so at the expense of latency. There's always a tradeoff between the quality, latency and stability of the audio signal in networked music contexts that has to constantly be negotiated (Fasciani, 2020).

In light of these facts, I would argue that it's more appropriate to expand and modify the definition of our measurement object to something more suitable, such as "the OOSE of a system that is also fine-tuned to produce stable and high-quality audio over a significant period with minimal dropouts". Let's call it _Stable_ OOSE, or SOOSE for short.

[BILDE]
OOSE delay chain
OOSE RTT measurement. input and output at the same place. OOSE round-trip.

However, measuring the SOOSE delay of networked music systems presents us with a key problem. How can we compare a delayed impulse to a reference if the delayed impulse only travels one way? We cannot send the reference because we want to isolate and measure the network path. The solution is to adopt a Round-Trip Time (RTT) strategy instead, where the delayed impulse is sent to a remote location and looped back again to the source location before being measured (Carôt 2009, Rottondi et.al 2016, Fasciani 2020). This strategy is similar to how all our computers negotiate clock time, actively measuring RTT latencies from fixed locations and subtracting the results from a common source. In NMP scenarios, doing RTT measurements requires us to loop back the delayed impulse at the very end of the SOOSE delay chain, preferably with a microphone capturing the sound from the speaker and sending it back to the source. If the signal chains at both locations are technically similar, with matching software and hardware configurations, dividing the RTT SOOSE by two will give you the one-way latency.
...
Below I have sketched out an example plan with four individual steps that, when followed, will give accurate and precise audio latency measurements. But first, let's discuss **when** you should conduct the measurements. If it's a one-time networked concert, I recommend doing all technical measurements on the day of the performance, for instance during the final soundchecks. Networks are fundamentally non-deterministic, meaning that the network conditions might change from day to day, even from hour to hour. Measuring right before a show is therefore the best indication of the true latency of the system at the time when it matters. Also, measuring during the final soundchecks might give you a head start on step number three.

[BILDE] SOOSE fra noe fra IRL?

### Step 1 - Verify The Measurement System

First thing is first, set up and verify the measurement system using the methods I've demonstrated in this post. Since this step does not require any network or music setup of any kind, it can be wise to do this before the music systems are set up. This is also a good time to run some bandwidth tests and configure any network settings.
...
### Step 3 - Test The Audio Quality

As stated, the key to unlocking a meaningful networked music practice is to achieve clear and stable audio transmissions. Therefore, once the measurement and music systems are connected, it's time to test the audio quality.

The simplest way to analyze the audio quality is to send a constant sine tone through the delayed signal chain and listen for any impurities or dropouts to the signal as it exits the local PA. This is an iterative process that involves adjusting various system settings until the audio is glitch-free, such as buffersize, bit-rate and sampling rate. I've written more extensively about this method in <MyLink href="/posts/how-to-improve-your-networked-audio-engineering">another post of mine on Networked audio engineering practices</MyLink>, about how to fine-tune audio parameters to suit the network.

[SINE TONE SYSTEM PHOTO]
...
[PICTURE OF MAXMSP SETUP] The Max software records all the latency metrics into one big text document (a simple [coll] object) that I can save and use in other software.
[OGSÅ HA EKSEMPEL FRA INTERNT]
