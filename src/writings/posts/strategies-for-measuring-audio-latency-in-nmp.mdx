...
type: "post"
summary: "Measuring audio latency in networked music performances (NMP) is stressful. There is little time and resources at hand, and on-site conditions often differ alot from controlled research setups. In this post, I share a few practical methods and strategies for measuring audio latency that guarantee precise and reliable data collection in the field, developed from years of professional experience."
---
...
Latency is a crucial factor for most networked art performances. If there is too much delay, communication and the sense of simultaneity between the performers breaks down. When playing music together, these tolerances and thresholds are especially low. In fact, current research dictates that even audio latency beyond 30-50ms is enough to disrupt the sense of simultaneity between two remote performers.
...
<MySlideshow
  ...
  captions={[
    ...
    "A loopback measurement example using a Hall Research AD-340 Universal Audio Delay Processor as my measurement object. Notice that the analog delay is set to 194 ms delay, the same latency as my Max software is registering on screen.",
...
Setting up networked performance systems is usually pretty hectic with many concerns and very little time to manage them. Having a detailed plan can ensure precision and scientific validity. And the better the plan is, the less you have to think and stress when the iron is hot. Below I have sketched out an example plan with four individual steps that, when followed, will give accurate and precise audio latency measurements.
