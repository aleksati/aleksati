...
const NavTop = ({ onToggleNavVertical, showNavVertical }: Props) => {
  ...
  // default to about
  const currRoute: string = firstRoute.length ? firstRoute : "aleksati.net";

  return (
    <nav className="z-50 fixed w-full bg-primary-light dark:bg-primary-dark" aria-label="Navbar" role="toolbar">
      <div className="flex p-4 items-center space-x-1">
        {/* This is the path link at the top */}
        {/* <p className="font-bold">/</p> */}
        <Link href={`/${currRoute === "aleksati.net" ? "" : currRoute}`} className="font-bold hover:cursor-pointer">
          {currRoute}
        </Link>
      </div>
      ...
      <div className="fixed top-3 right-4">
        <ButtonIcon iconId={showNavVertical ? "x" : "threedots"} onClick={onToggleNavVertical} />
      </div>
    </nav>
  );
};
