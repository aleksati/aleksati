...
1. <MyLink href="#follow-the-code">Dataset Collection and Exploration</MyLink>
2. <MyLink href="#why-sonify?">The Models</MyLink>
3. <MyLink href="#example-dataset">Results</MyLink>
4. <MyLink href="#summary">Summing up</MyLink>

---

# Dataset Collection and Exploration

The project structure can be divided into .. parts with the first, and arguably most essential, segments associated with gathering and pre-processing of the dataset.

PICTURE OF DIAGRAMS

To interface with Spotify's Web API, I used a Python library called Spotipy. This library is good because it handles all API call complexity, making it even easier to retrieve data. All you need is to create a user over at Spotify Dev for authentication and gather the track/playlist/user IDs for data retreival.

But simply gathering a bunch of track data is not enough. To create model capable of predicting musical preferances, you need a dataset where each item is associated with a rating or target value.

Insted of doing it on my own.

got tracks from getTop tracks
hack to make it 300
how did I rate them.
