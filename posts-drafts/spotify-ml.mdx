...
Browsing the web in the 21st century is a bitter-sweet. Our (browsing) time and behavior has become a stready revenue stream for companies looking to give us better advertisement. But it's not all ads in the traditional sense. For instance, in the music industry, big data is used to create personalized playlists and "smart" music recommendation systems. Nobody is better at this than Spotify, currently the largest music streaming service in the known universe. For a long time, Spotify's recommendation systems have fascinated me due to their sheer performance and ability to reveal musical taste (quantitatively) like never before.
...
The project structure can be divided into .. parts, as demonstrated in the figure below. The first, and arguably most essential, part is concerned with gathering data and pre-processing it into a dataset for my models. To interface with the Spotify Web API, you need an account over at Spotify Dev for authentication. I also recommend using a Python library called Spotipy that effectively handles the API contact to Spotify endpoints. Using Spotipy frees up valuable time and code to navigate the intricate data structures returned by the API endpoints.

PICTURE OF DIAGRAMS

Simply gathering a bunch of tracks and features is not enough to build a good model for predicting musical preferences. It also requires unique scores (target values) associated with each item in the dataset that represents how much the user enjoys each track. However, acquiring target values is not as straightforward as it sounds. Just think about it. How would you accurately and systematically rate a playlist of your favorite music? Is it even possible to precisely articulate our musical preferences?
