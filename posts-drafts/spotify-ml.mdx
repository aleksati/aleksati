...
Ok, new plan. Do what I did before, but now, use all music! and get the results from "top items" to provide ratings.

Then, create a regressor that would predict my liking.

Create Personal Recommandation Model Using Spotify Web API.

Do to:

- gather top 1000 tracks in chronological order in a list.
- use trackIDs to extract features.
- use the order/popularity of the track to determine the rating
- feed into MLP regressor.
