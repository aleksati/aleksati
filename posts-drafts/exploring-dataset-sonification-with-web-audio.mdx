...
type: "posts"
summary: "Thanks to modern browser tech and open source creative coding libraries, creating audio-visual art on the Web is easier than ever. In this post, I share some examples of how to enrich your dataset visualizations with model-based sonification in a web browser using the p5js library."
---
...
If you want to follow my code examples, you can <MyLink href="https://github.com/aleksati/dataset-sonification-in-browser-workshop">download the repository from my GitHub</MyLink>. I recommend using the <MyLink href="https://editor.p5js.org/">p5js Web Editor</MyLink> as your development environment so you can focus 100% on coding. However, you can also run the code from a local node development server, if you want.
...
Below is my first example of a model-based sonification system where I've augmented a traditional 2D plot with web audio. Each red dot reprents unique coffee dispatches; rows in the dataset spreadsheet. The height (vertical Y-axis) of the red dots equals coffee acidity levels over time (horizontal X-axis).

When you click the dots, the frequency of the played synth note represents the altitude of the farm. The higher the frequency of the note, the higher up the farm is. The farm name is also printed for reference.
...
// The frequency of a sinewave is determined by a
// values in a dataset csv column.

const sketch = (p5) => {
  let data, data_col, i, sine;
  let threshold = 10000;

  // The preload method is called first.
  p5.preload = () => {
    // load a dataset .csv file
    data = p5.loadTable("PATH/TO/DATASET");
    // get a specific column of the dataset as an array.
    data_col = data.getColumn("CSV COLUMN NAME")
  }

  // the setup method get called once at the start of a new runtime.
  p5.setup = () => {
    // create a canvas.
  ...
  // the draw method is called in loop on every new window frame.
  p5.draw = () => {
    // reset increment variable if necessary
    i = i > data.length ? 0 : i
    // update the frequency of the sine with data value.
    sine.freq(data_col[increment]);
    // increment i
    i++
  };
};
...
By listening to the frequency of the LFO and the varying characteristics of the synth notes, we can disseminate information about how many bags of coffee were part of every shipment. The higher the frequency of the LFO, the more coffee bags were produced. See if you can hear the differences.
...
Below is another example of a model-based sonification system with a more creative and object-oriented approach to visual and sonic design. Here, users can navigate through time by interacting with a slider, moving colored orbs around in a 2D plot.

Each orb represents a unique company in the dataset. Selected information about the company's coffee dispatches are mapped to various properties of the orb. In my particular example, the XY position of the orbs communicate the "balance" and "body" of coffee dispatches from 3 companies between 2010 - 2012. For simplicity's sake, I also made the sound of the orbs communicate the same thing using the same sound design as my multi-dimensional plot example, i.e. amplitude modulation.

<P5SonicOrbs />

In the code, I define an Orb class with some utility methods where each class instance represents a unique coffee owner/company from the dataset. So every Orb has the same basic features, such as shape, size, movement, sound, etc.

But any data point can be mapped to any property of these orbs. When I create a "new Orb" I simply pass whatever data I want to see and hear as arguments and ensure that the current value of my slider is passed to every Orb in the draw function.
...
const sketch = (p5) => {
  ...
  p5.setup = () => {
    p5.createCanvas(200, 200);
    // create a HTML slider
    slider_html = p5.createSlider(0, 100, 0);
    // initialize a new Orb
    orb1 = new Orb(owner_1, color_1, size_1);
  };

  p5.draw = () => {
    // get the current slider value
    sliderVal = sliderHTML.value();
    // update my orbs on every new window frame.
    orb1.update(data_balance[sliderVal], data_body[sliderVal]);
    orb1.draw();
    orb1.play();
  };
};
...
// My orb class where I define the various behavior of my orbs.
class Orb {
...
So there you have it; three examples of how to navigate and explore Big data on the web using model-based sonification. More specifically, my post demonstrates two ways we can build systems with the p5js library that augment visual displays of datasets, capable of revealing more dimensions in realtime. The first approach is more traditional, using audio to augment standard 2D visual graphs, while the other design is more experimental, with usual and complex visual-audio aspects.

The biggest upside of these web apps is that they can facilitate more diverse, accessible and multi-faceted forms of online interaction with complex data. However, there is no guarantee that such apps actually work as intended. In addition to being visual aids, sonification should be highly data-driven, consistent and customizable in these contexts. It's also important to remember that we interpret sound very individually. So considering that most visual software offer features to customize data visualization based on preference, I think we should aspire to offer similar kinds of user control of audio or synthesis parameters in sonification-based apps.

Finally, although p5js is great for building easy-to-use, audio-visually robust web apps, it's not really a library designed to be the backbone of fast and powerful analysis software. In other words, programming analysis-heavy programs with p5 is not really recommended. Rather, it's great for prototyping and smaller apps/projects.

Be sure to check out [my GitHub page](https://github.com/aleksati/workshop-in-dataset-sonification-with-web-audio) for the full source code.
