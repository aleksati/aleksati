...
type: "posts"
summary: "Thanks to modern browser tech and open-source creative coding libraries, creating audio-visual art on the Web is easier than ever. In this post, I share some basic examples of how to enrich data visualizations with model-based sonification in a web browser using the p5js library."
---
...
In summary, it's very much possible to enrich data visualizations with model-based sonification in a web browser using the p5js library. In my post, I demonstrate two approaches for this that use realtime audio to augment visual displays as a way to reveal more dimensions. The first approach can be considered more traditional, using audio to augment simple 2D visual graphs, while the second approach is more experimental, with usual and complex visual-audio aspects.

With all artistic and funny-business aside, I think the biggest advantage of these audio-visual apps is that they can facilitate more diverse, accessible and multi-faceted forms of online interaction with complex data. However, it's important to note that there is no guarantee that this is the case. Doing proper user-testing of these applications is a tedious and complicated process, usually requiring a large variety of qualitative and quantitative data to be definitive. People also interpret sounds and audio very subjectively, making matters even worse. But from my experience from working on this project, I believe you can come a long way by designing these kinds of apps to be highly data-driven, consistent, and interactive(!). The web provides a unique platform for large-scale testing and distributing to people of all demographics, so interactivity is key.
