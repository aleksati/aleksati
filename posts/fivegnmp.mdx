...
date: "2023-02-10"
keywords: ["audio", "networking", "5G", "analysis"]
...
By using the <MyLink href="https://iperf.fr/">iPerf</MyLink> networking utilities, the Huawei routers's own location-optimizing software, <MyLink href="https://www.telenor.no/dekning/#dekningskart">Telenor’s online coverage map</MyLink>, and <MyLink href="https://www.speedtest.net/">Ookla's online speedtester</MyLink>, we were able to make network bandwidth and coverage estimates throughout the experiments, ensuring that our load did not exceed the capacity of the network.
...
By including this additional hardware, you measure the analog RTT. To achieve higher accuracy and certainty in our analog RTT measurements, we used a secondary pc+soundcard setup, as shown in the diagram below.
...
Since the 5G reception inside the Musicology department at UiO was poor, we decided to place the routers outside hoping it would generate better coverage and boost overall performance.

Just outside IMV's main auditorium (at <MyLink href="https://www.google.com/maps/place/59%C2%B056'13.4%22N+10%C2%B043'13.9%22E/@59.9370556,10.7205278,226m/data=!3m2!1e3!4b1!4m4!3m3!8m2!3d59.9370556!4d10.7205278">this location</MyLink>) we measured a stable 75% 5G coverage with 60Mbps of bandwidth.

During our tests we experienced massive dropouts and periodic glitches when using sofware buffersizes below 512. Unfortunately, this eliminated the possibility of using Lola. Using JackTrip, we were able to find a good quality+stability sweetspot using the following settings:
...
With the above configuration using JackTrip on a commercial 5G network at UiO, we measured a 110ms digital RTT and 165ms analog RTT audio latency. Considering that buffering 512 samples at 48Khz takes 10ms (and our audio had to buffer 4x times during our experiment), this difference between digital and analog RTT was expected.
...
During our second round of testing on a private 5G in Elverum, time was of the essence. We only had about 3-4 hours to set up our equipment and ran into unexpected connectivity and bandwidth issues from the very start. These issues meant we again had to eliminate using Lola and settle for limited, but fair, bandwidth (approximately 13-14Mbps).

On the private 5G, we were able to get stable and good-quality audio using lower buffersizes than on the commercial. At 256, we got decent audio with some periodic dropouts over time. We found the optimal stereo audio settings to be the following:
...
With the above configuration using JackTrip on a private 5G network, we measured a 55-60ms digital RTT and an analog RTT at 74ms. Although impressive, this configuration rendered borderline audio quality that would be unpleasant in the long run. When using a buffersize of 512, we measured a 90-100ms digital RTT (similar to experiment nr.1, only slightly faster) and analog RTT at about 140ms.
...
In early 2022, me and a UiO collegue investigated the feasibility of conducting NMPs over commercial and private 5G networks in collaboration with Telenor Research. On two separate occasions, we measured the stability, quality, and latency of transferring uncompressed audio over the networks with high-end hardware and software utilities. When accepting borderline conditions on a private 5G network (5GNoW), we managed to push the audio RTT latency down to 75ms. However, in more realistic conditions on a first-generation commercial 5G, we achieved an analog RTT audio latency of 165ms.
