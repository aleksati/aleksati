...
type: "post"
summary: "We teamed up with Telenor Research to see if it's going to be possible to play music together over next generation 5G networks. Here are the preliminary results from our first audio experiments on 5G."
---
...
**Demo** - For perspective, here is what 30ms of audio delay sounds like from left to right ear:

<MyAudioPlayer src="fiveg-30ms-delay.mp3" />

In 2022, me and my colleague (<MyLink href="https://stefanofasciani.com/">Stefano Faciani</MyLink>) got in touch with Telenor Research to explore the feasibility of conducting NMPs over 5G. During spring, we were able to schedule and complete a series of audio experiments on private and commercial 5G networks. In this post, I present our experiments and results in detail.
...
To transmit the audio over 5G, we used a pair of custom-built <MyLink href="https://github.com/MCT-master/NMP-Portable-Kits/wiki">NMP hardware kits</MyLink> from IMV. These hardware bundles are designed specifically for ultra-low-latency AV transmission making them perfect for NMP-related education and research.

The kits use high-end RME PCI audio interfaces connected to Behringer ADAT converters for XLR inn/out.

On the software side, we settled on using <MyLink href="https://www.jacktrip.org">JackTrip</MyLink> and Qjack from the Jack-audio connection kit as our transmission software.
...
Having lightning-fast communication is always a high priority for any NMP. However, low latency is not synonymous with high-quality transmissions with minimal unwanted artifacts (glitches) and dropouts.

In light of this, we tried to find the best possible tradeoff between stability, quality, and latency before we ran our measurements. Instead of simply benchmarking various metrics, we were more intersted in finding suitable conditions where musical activity actually could occur.
...
<MyImage
  src="5G-latency-method-third-laptop.png"
