...
Finally, we needed lightweight transmission software that was agnostic about network conditions, could support uncompressed formats, and worked well cross-platform. Here, <MyLink href="https://www.jacktrip.org">JackTrip</MyLink> from the Jack-audio connection kit seemed to fit the bill.

For the record, we also did experiments with the software called LoLa but never managed to connect the two 5g routers using it.
