...
The yield function also handle looping for us as it returns to start (0th index item) when iterating past the last array item (-1st index).
...
In the above example, I pass a desired scene (array) from scene_config into the nextTrack function. The function returns a generator object I can use to asynchronously iterate over the array using the next() function.
