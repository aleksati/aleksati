...
summary: "While designing a new liveset for my band, I wrote some code to help me navigate Ableton Live more efficiently using very few midi controls. Here is how I did it."
---
...
<ol>
  ...
  <ol>
    ...
    <li>
      <MyLink href="#skipping-audio-tracks">
        Skipping Audio Tracks with Yield
      </MyLink>
    </li>
    <li>
...
<h3 id="skipping-audio-tracks">Skipping Audio Tracks with Yield</h3>

To skip between audio tracks, I iterating and loop over the scene_config arrays asynchronously in response to keypresses from my USB controller. To handle this kind of functionality, I recommend using the \_yield\* Generator function in Python.
...
I repeated the same code for the output initialization, only substituting rtmidi.MidiIn() with rtmidi.MidiOut() and using the virtual midi driver instead.
...
**Pro tip:** If you're unsure about what kind of messages or midi values your devices in sending, you can print out raw midi messages from your rtmidi connection with a simple keyboard-test script, like this:
...
In Ableton, all I have to do is midi map my project and enable the right midi setting.
...
<MyImage
  ...
  caption="Making a `daw-midi-mapping.py` is recommended when your Ableton setup is tighly coupled with Python variables."
/>

Finally, I have to configure the correct midi settings. Per default, Ableton will recognize and enable any compatible midi device it detects. However, I only want Ableton to listen to the data from Python.

To open preferences, Hit **"ctrl" + ","** and navigate to the midi section. The only midi input I want is from the virtual midi driver, so I uncheck every other device.
