...
4. <MyLink href="#the-sonic-snake">The Sonic Snake</MyLink>
...
With these utilities, and some basic knowledge of sound synthesis, we can start to build more complex sounds that can tell us more about our data. Below is an upgraded example of the red dot (now blue) coffee plot that features a second oscillator used as an LFO to control the amplitude of the first oscillator, creating an amplitude modulation synthesizer in the process.
...
Although ....
re-creating traditional plots and graph structures with p5 is tedious work. Unlike libraries such as Plotly and Matplotlib, p5 is not optimized nor designed for data visualizations and statistical analysis (with a few exceptions, like <MyLink href="https://github.com/jagracar/grafica.js">grafica.js</MyLink>). So instead of replicating traditional plotting techniques and functionalities, we should try to take advantage of the strong visual capabilities and object-oriented affordances of p5 and create more tailored platforms.
...
In the code, I define a global class where each instance of the class represents an indvidual coffee owner/company. Every class instance (usually refered to as an object) posesses a range of properties that are defined by the coffee owners data points in the dataset. In my example, I use class properties that define the shape, color, sound, position, and body length of my object instances.

Something about the sound..
every body-part has a unique oscillator with a frequency.

```Javascript
// A simplified version of my Snake class, only demonstrating how I
// initialize a new oscillator for every new body part I recevie and store
// them in seperate object variables.
class Snake {
  constructor(owner, point_size, rgbcolor=[]) {
    this.body = []; // array of positions. [[x, y], [x, y], [x, y]]
