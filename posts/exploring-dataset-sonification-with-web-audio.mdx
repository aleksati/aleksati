...
// The frequency of a sinewave is determined by
// values in a column of a .csv dataset
...
const sketch = (p5) => {
  ...
  p5.preload = () => {
    data = p5.loadTable("PATH/TO/DATASET"); // load a dataset .csv file
    data_col = data.getColumn("CSV COLUMN NAME")  // get a specific column of the dataset as an array.
  }

  // the setup method get called once at the start of a new runtime.
  p5.setup = () => {
    p5.createCanvas(200, 200); // create a canvas.
    sine = new p5.Oscillator("sine");  // load an oscillator with a sinewave and set initial amplitude.
    sine.amp(0.5);
    sine.freq(0);
    sine.start();
  };
  ...
  p5.draw = () => {
    i = i > data.length ? 0 : i // reset increment variable if necessary
    sine.freq(data_col[increment]);  // update the frequency of the sine with data value.
    i++ // increment i
  };
};
...
But let's take this one step further. Below is an upgraded example of the red dot (now blue) coffee plot that features a second audio oscillator. The additional oscillator is used as an LFO to control the amplitude of the first oscillator, creating an amplitude modulation synthesizer in the process.
