...
My contribution to the project was to ensure that the drywall sculpture reacted to its surrounding WiFi traffic, using Bluetooth transducers in combination with MaxMSP (Max) and Python software.
...
# Internal IP (localhost) port that Max will receive OSC messages from python on.
max_port = 8888
max_ip = "127.0.0.1"
...
if __name__ == "__main__":
    ...
    for pkt in cap:
        ...
        if (prev_pkt_len != pkt.length):
            ...
            # If the packet has a property called IP
            if "IP" in pkt:
                ...
                try:
                    # If the source IP of the packet matches my ip_host
                    if source == ip_host:
                        # send stuff to Max
