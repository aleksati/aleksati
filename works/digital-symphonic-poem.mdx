...
Finally, being executed at audio rate, my Kuramoto model required some extra scaling and fine-tuning. In particular, I had to scale the coupling constant and natural frequencies down to get smooth performance. Here is an example:

```C
// inside [mc.gen~ ksync_node] codeblock
// Scaling the k_value and n_freq to the current sampling rate resulted in smooth performance at audio rate
...
My model outputs sine ways, I needed a way to turn this into audio.
